"""MUSE Stage 1 Training: Topology Warmup.

Narrative: "Establishing the Structural Manifold"
    - Freezes: InternViT Backbone, Semantic Projector (ITC).
    - Trains: MUSE Synergy Blocks, Adapter, Decoder.
    - Objectives: Pixel Reconstruction + Topological Homeomorphism (KL-DINO).
    - Analysis: Monitors Gradient Orthogonality between Pixel and Structure losses.

Usage:
    accelerate launch scripts/train_stage1.py config=configs/muse_1b/stage1.yaml
"""
...
from muse.utils.train_utils import (
    get_config,
    create_model_stage1,
    create_optimizer_standard,
    create_lr_scheduler,
    create_dataloader,
    create_evaluator,
    auto_resume,
    save_checkpoint,
    train_one_epoch_stage1,
)
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        logger.info(">>> MUSE Stage 1: Topology Warmup Started.")
    ...
    # --- 2. Model & Loss ---
    model, ema_model, loss_module = create_model_stage1(config, logger, accelerator)

    # --- 3. Optimizer & Scheduler ---
    optimizer, discriminator_optimizer = create_optimizer_standard(config, logger, model, loss_module)
    ...
    if discriminator_optimizer:
        (
            model, loss_module, optimizer, discriminator_optimizer,
    ...
    logger.info("***** Running MUSE Stage 1 Training *****")
    logger.info(f"  Narrative Goal = Establish Topological Manifold (DINO Align)")
    ...
    for current_epoch in range(first_epoch, num_train_epochs):
        ...
        global_step = train_one_epoch_stage1(
        ...
        if global_step >= config.training.max_train_steps:
            accelerator.print(f"Finishing Stage 1: step {global_step} >= {config.training.max_train_steps}")
            break
    ...
    logger.info(">>> MUSE Stage 1 Completed Successfully.")
