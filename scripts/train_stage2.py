"""MUSE Stage 2 Training: Active Semantic Injection.

Narrative: "Breaking the Impossible Triangle"
    - Loads: Stage 1 checkpoint (Structure Warmup).
    - Freezes: InternViT Backbone.
    - Unfreezes: Semantic Projector (ITC), Logit Scale.
    - Objectives: Rec + Topo + ITC (Image-Text Contrastive).

Usage:
    accelerate launch scripts/train_stage2.py config=configs/muse_1b/stage2.yaml
"""
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        logger.info(">>> MUSE Stage 2: Active Semantic Injection Started.")

    if config.training.seed is not None:
        set_seed(config.training.seed, device_specific=True)
    accelerator.wait_for_everyone()

    model, ema_model, loss_module, tokenizer, text_encoder = create_model_stage2(
    ...
    logger.info("***** Running MUSE Stage 2 Training *****")
    ...
    for current_epoch in range(first_epoch, num_train_epochs):
        ...
        global_step = train_one_epoch_stage2(
            config, logger, accelerator,
    ...
    logger.info(">>> MUSE Stage 2 Completed Successfully.")
